...
from src.orchestration.runtime_utils import (
    new_runtime_profile,
    record_stage,
    append_runtime_span,
    runtime_image_size,
    stage_name_for_context_type,
    classify_runtime_error,
    first_non_empty,
)
from src.orchestration.review_reasons import (
    warning_requires_human_review,
    review_reasons_from_answer_warnings,
    answer_requires_extraction_review,
    review_reasons_from_report,
    extend_question_review_reasons,
    merge_review_reasons,
)
from src.orchestration.image_processing import (
    image_chunk_plan,
    desired_answer_region_concurrency,
    perception_timeout_seconds_for_context_type,
)
...
class PaperGradingWorkflow:
    ...
    async def run_pipeline(
        ...
        record_stage(runtime_profile, "file_preprocess", time.perf_counter() - start)
    ...
    async def run_pipeline_with_preprocessed_images(
        ...
        *,
        ...
        runtime_profile = runtime_profile or new_runtime_profile()
        ...
        record_stage(runtime_profile, "student_page_ocr", time.perf_counter() - stage_start)
        ...
        record_stage(runtime_profile, "student_page_layout", time.perf_counter() - stage_start)
        ...
        record_stage(runtime_profile, "split", time.perf_counter() - stage_start)
        ...
        record_stage(runtime_profile, "answer_region_preprocess", time.perf_counter() - stage_start)
        ...
        record_stage(runtime_profile, "answer_region_ocr", time.perf_counter() - stage_start)
        ...
        record_stage(runtime_profile, "artifact_write", time.perf_counter() - artifact_start)
        ...
        record_stage(runtime_profile, "total", time.perf_counter() - total_start)
    ...
    async def _build_paper_report_from_answer_bundle(
        ...
        *,
        ...
        for rubric in rubric_bundle.rubrics:
            ...
            if answer.extraction_warnings:
                ...
                extend_question_review_reasons(
                    ...
                    rubric.question_id,
                    review_reasons_from_answer_warnings(answer.extraction_warnings),
                )
            if answer_requires_extraction_review(answer):
                warnings.append(
        ...
        if pending_rubrics:
            ...
            record_stage(runtime_profile, "cognitive_evaluation", time.perf_counter() - stage_start)
            for rubric, report in zip(pending_rubrics, reports):
                if REVIEW_REASON_EXTRACTION_RISK in per_question_review_reasons.get(
                    ...
                    report.review_reasons = merge_review_reasons(
                ...
                report.review_reasons = merge_review_reasons(
                    ...
                    review_reasons_from_report(report),
                )
        ...
        review_warnings = [
            warning for warning in warnings if warning_requires_human_review(warning)
        ]
        return PaperEvaluationReport(
            ...
            review_reasons=merge_review_reasons(paper_review_reasons),
            warnings=warnings,
            per_question=per_question,
    ...
    async def _process_images_in_chunks(
        ...
        *,
        ...
        chunk_size, chunk_concurrency = image_chunk_plan(
        ...
        *,
        ...
        try:
            outputs = await asyncio.wait_for(
                ...
                timeout=perception_timeout_seconds_for_context_type(context_type),
            )
        except asyncio.TimeoutError:
            calls = self._perception_engine.end_runtime_capture(capture_token)
            append_runtime_span(
                runtime_profile,
                stage=stage_name_for_context_type(context_type),
        ...
        for span in spans:
            ...
            if isinstance(item_refs, list):
                for item in item_refs:
                    ...
                    if question_id:
                        question_reasons[question_id] = merge_review_reasons(
                            question_reasons.get(question_id, []),
                            [reason],
                        )
                        assigned = True
            if not assigned:
                paper_reasons = merge_review_reasons(paper_reasons, [reason])
        return question_reasons, paper_reasons, warnings
    ...
    async def _evaluate_question_from_perceptions(
        ...
        *,
        ...
        if global_confidence < 0.80:
            ...
            evaluation_report.review_reasons = merge_review_reasons(
