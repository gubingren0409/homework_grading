...
def test_router_degrades_on_failure_rate_threshold(monkeypatch):
    ...
    router.record_event(model=settings.deepseek_model_name, success=False, token_estimate=100, fallback_used=False, reason="x")
    router.record_event(model=settings.deepseek_model_name, success=False, token_estimate=100, fallback_used=False, reason="x")
    router.record_event(model=settings.deepseek_model_name, success=True, token_estimate=100, fallback_used=False, reason="x")
    ...
    assert decision.cognitive_model == settings.deepseek_fallback_model_name
...
def test_router_follows_default_when_healthy(monkeypatch):
    ...
    assert decision.cognitive_model == settings.deepseek_model_name
...
def test_router_snapshot_includes_hit_breakdown(monkeypatch):
    ...
    monkeypatch.setattr(settings, "auto_circuit_controller_enabled", True)
    router = RuntimeRouterController()
    router.record_event(model=settings.deepseek_model_name, success=True, token_estimate=110, fallback_used=False, reason="ok")
    router.record_event(model=settings.deepseek_fallback_model_name, success=False, token_estimate=130, fallback_used=True, reason="parse_error")
    snap = router.snapshot()
    assert snap["sample_count"] == 2
    assert snap["fallback_trigger_count"] == 1
    assert snap["model_hits"][settings.deepseek_model_name] == 2
